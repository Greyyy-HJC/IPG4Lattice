...
- `scripts/qprop_mom.py`: point-source time-direction quark propagator with spatial momentum projection via `MomentumPhase`. Plots effective masses for all five gamma insertions (I, gX, gY, gZ, gT) at each momentum.
...
IPG only constrains the spatially-averaged temporal links `u(t)`, so only the spatially-averaged (wall-source) time-direction quark propagator carries a clean signal after IPG. The tdir propagator in `qprop_static.py` therefore uses a wall source; the momentum-projection and Green's-function scripts use a point source at the origin.
...
- `scripts/qprop_mom.py`: point-source tdir quark propagator with spatial momentum projection via `MomentumPhase`. Uses all five gamma insertions (I, gX, gY, gZ, gT) and plots the effective mass for each momentum.
