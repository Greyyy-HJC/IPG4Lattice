...
- `scripts/qprop_dressing.py`: coarse-grid dressing diagnostics (20 4-momenta: 4 spatial shells × 5 temporal). Point source + lattice FFT, scalar `As`/`Bm`/`M` per momentum and index plots. Shared measurement helpers live in `scripts/qprop_M.py`.
- `scripts/qprop_M.py`: dense-grid paper-style `M(|k|)` from FFT greens (125 4-momenta, Eq. 25 p4 average). Supports staggered or clover fermions and a bare-mass scan via `mass_ls`. Outputs `artifacts/data/qprop_M_{fermion}_{ensemble}.npz` and `artifacts/plots/qprop_M_{fermion}_{ensemble}.pdf`.
...
- `scripts/qprop_dressing.py`: coarse momentum grid for per-point dressing and p4-slice diagnostics before Eq. 25 averaging. Edit the parameter block at the top and run with PyQUDA (`python scripts/qprop_dressing.py`).
- `scripts/qprop_M.py`: dense cylinder-cut shell grid for `M(|k|)` curves and bare-mass scans (staggered or clover). Edit `fermion`, `mass_ls`, and related parameters at the top; run with `python scripts/qprop_M.py`.
...
Run dressing diagnostics and paper-style `M(|k|)` (PyQUDA / GPU required):

```bash
/home/jinchen/miniconda3/envs/pygpt/bin/python scripts/qprop_dressing.py
/home/jinchen/miniconda3/envs/pygpt/bin/python scripts/qprop_M.py
```
