...
- Added `tests/qprop_dressing.py` post-processing to convert momentum-space quark propagator traces into Eq. 20 inverse-propagator dressing functions `As`, `At`, `Bm`, `M = Bm / As`, and spatial-Wilson-term-corrected `M_corr`, with `As`/`At` dimensionless and mass quantities in lattice units.
